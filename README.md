...
```bash
wsl
docker run --name macos -id --device /dev/kvm -p 50922:10022 -p 5999:5999 -v "/run/user/$(id -u)/pulse/native:/tmp/pulseaudio.socket" -e PULSE_SERVER=unix:/tmp/pulseaudio.socket -v /tmp/.X11-unix:/tmp/.X11-unix -e AUDIO_DRIVER=pa,server=unix:/tmp/pulseaudio.socket -e "DISPLAY=${DISPLAY:-:0.0}" -e GENERATE_UNIQUE=true -e MASTER_PLIST_URL='https://raw.githubusercontent.com/sickcodes/osx-serial-generator/master/config-custom.plist' sickcodes/docker-osx:ventura
```
- VNC Viewer
